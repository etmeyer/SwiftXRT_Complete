...
**Software:**
- Python 3.9+
- [HEASoft](https://heasarc.gsfc.nasa.gov/lheasoft/) (provides `xselect`, `xrtexpomap`, `xrtmkarf`, `grppha`)
- [CALDB](https://heasarc.gsfc.nasa.gov/docs/heasarc/caldb/) for Swift XRT
- [Sherpa](https://sherpa.readthedocs.io/) (via [CIAO](https://cxc.cfa.harvard.edu/ciao/) or `pip install sherpa`) — for spectral fitting only
...
Clone or download the repository, place the scripts somewhere permanent, make them executable, and add the directory to your `PATH`:
...
setup_swiftxrt() {
    ...
    if [ ! -d "$pipedir" ]; then
        echo "ERROR: $pipedir not found."; return 1
    fi
    case ":$PATH:" in
        *":$pipedir:"*) ;;
        *) export PATH="$pipedir:$PATH" ;;
    esac
    if command -v swift_xrt_summary.py &>/dev/null; then
        local n=$(ls "$pipedir"/*.py 2>/dev/null | wc -l)
        echo "Swift XRT Pipeline ready ($n scripts in $pipedir)"
    else
        echo "WARNING: scripts not executable. Run: chmod +x $pipedir/*.py"
        return 1
    fi
}
```

Then activate in any terminal session:
